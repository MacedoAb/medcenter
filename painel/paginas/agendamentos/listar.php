...
echo <<<HTML
		...
		<p>Confirmar Agendamento? <a title="Confirmar Agendamento" href="#" onclick="confirmar('{$id}', '{$horaF}')"><span class="text-success">Sim</span></a></p>
        			...
        			<a class="" href="#" onclick="editar('{$id}','{$cliente}','{$funcionario}','{$servico}',
					'{$data}','{$obs}','{$retorno}')" title="Editar Agendamento" class=""> <img class="icon-rounded-vermelho" src="img/editar.png" width="15px" height="15px"></a>
...
<script type="text/javascript">
	function baixar(id, cliente, servico, valor_servico, convenio, funcionario, id_servico, retorno){
		...
		$('#procedimento').text("Consulta ");

		if(retorno == "Sim"){
			$('#valor_serv_agd').val("0");
			$('#procedimento').text("Retorno Consulta");
		}else{
